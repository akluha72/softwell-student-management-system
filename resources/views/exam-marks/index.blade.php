...
@section('content')
    ...
    {{-- Search bar --}}
    <form method="GET" action="{{ route('exam-marks.index') }}" class="mb-4">
        <div class="flex gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ $search }}"
                    placeholder="Search by student name, student ID, course name or code..."
    ...
    @if ($search)
        <p class="text-sm text-gray-500 mb-4">
            {{ $examMarks->total() }} result{{ $examMarks->total() !== 1 ? 's' : '' }} for
